...
function LoadCasinoJob() {
  ...
  $.post(
    "https://qb-phone/CheckHasBetTable",
    JSON.stringify({}),
    function (HasTable) {
      if (JSON.stringify(HasTable) != "[]") {
        ...
        $(".casino-list").html("");
        var AddOption =
          '<div class="casino-text-clear">No Event</div>' +
          "<br />" +
          "<br />" +
          "<br />" +
...
function AddToChat(data) {
  ...
  if (JSON.stringify(data) != "[]") {
    for (const [k, v] of Object.entries(data)) {
      ...
      var Fulltext =
        firstLetter.toUpperCase() + v.Name.replace(firstLetter, "");

      var AddOption =
        '<div class="casino-license-body-main">' +
        '<div class="casino-license-text-class">' +
        Fulltext +
        "</div>" +
        '<div class="casino-license-icon-class"><i data-name="' +
        Fulltext +
        '" data-chanse="' +
        v.chanse +
        '" data-id="' +
        v.id +
        '" id="casino-click-beting" class="fas fa-coins"></i></div>' +
        "</div>";
      $(".casino-list").append(AddOption);
    }
  } else {
...
function CheckStatus() {
  $.post(
    "https://qb-phone/CheckHasBetStatus",
    JSON.stringify({}),
    function (HasStatus) {
      if (HasStatus) {
...
$(document).on("click", "#casino-end-task-accept", function (e) {
  ...
  if (Amount != "" && Amount >= 1.0) {
    $.post(
      "https://qb-phone/BettingAddToTable",
      JSON.stringify({
        amount: Amount,
