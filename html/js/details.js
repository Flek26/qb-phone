function LoadPlayerMoneys() {
  ...
  var PlayerPaycheck = QB.Phone.Data.PlayerData.money.paycheck;
  ...
  $(".details-paycheck").html("$" + PlayerPaycheck);
  ...
  for (const [k, v] of Object.entries(PlayerLicenses)) {
    if (v) {
      ...
      var Fulltext =
        firstLetter.toUpperCase() + k.replace(firstLetter, "") + " License";

      var AddOption =
        '<div class="details-license-body-main">' +
        '<div class="details-license-text-class">' +
        Fulltext +
        "</div>" +
